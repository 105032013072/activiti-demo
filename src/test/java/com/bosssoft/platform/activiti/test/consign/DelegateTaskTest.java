...
public class DelegateTaskTest extends AbstractTaskTest{
	...
	public void sourceCommon() throws Exception{
		...
		taskService.claimCascade(btask.getTaskId(), getConsignInstIdForClaim(btask.getTaskId(), "u04"), "u04");
		...
		taskService.claimCascade(ctask.getTaskId(), getConsignInstIdForClaim(ctask.getTaskId(), "u06"), "u06");
		...
		for (TaskExt taskExt : list) {
			...
			taskService.completCascade(taskExt.getTaskId(), getConsignInstIdForComplete(taskExt.getTaskId()));
		}
	...
	//	taskService.claimCascade(dtask.getTaskId(), dtask.getConsigInstId(), "u06");
		taskService.claimCascade(dtask.getTaskId(), getConsignInstIdForClaim(dtask.getTaskId(), "u06"), "u06");
		...
		taskService.unclaimCascade(dtask.getTaskId(), getConsignInstIdForUmClaim(dtask.getTaskId(), "u06"));
	...
	public void sourceDelegate() throws Exception{
	   ...
	   taskService.claimCascade(ctask.getTaskId(), getConsignInstIdForClaim(ctask.getTaskId(), "u10"), "u10");
	   ...
	   verifyEntity=new TaskVerifyEntity("B", TaskBusinessState.DELEGATE.toString(), ConsignInstState.UNFINISH.toString(), null, true, false,"u02");
	   ...
	   verifyEntity=new TaskVerifyEntity("C", TaskBusinessState.DELEGATE.toString(), ConsignInstState.UNFINISH.toString(), "u08", true, true,"u08");
	 ...
	 /*  taskService.completCascade(ctask.getTaskId(), ctask.getConsigInstId());
	   ...
	   taskService.completCascade(ctask.getTaskId(), getConsignInstIdForComplete(ctask.getTaskId()));
	   taskService.completCascade(btask.getTaskId(), getConsignInstIdForComplete(btask.getTaskId()));
	 ...
	 //  taskService.claimCascade(dtask.getTaskId(), dtask.getConsigInstId(), "u10");
	   ...
	   taskService.claimCascade(dtask.getTaskId(), getConsignInstIdForClaim(dtask.getTaskId(), "u10"), "u10");
	  ...
	  // taskService.unclaimCascade(dtask.getTaskId(), dtask.getConsigInstId());
	   taskService.unclaimCascade(dtask.getTaskId(), getConsignInstIdForUmClaim(dtask.getTaskId(), "u10"));
	   verifyUnfinishTask(TestTaskClassify.ALL, 0, "u08", null);
	...
	public void sourceSubsitution() throws Exception{
       Model model=super.createAndDeployModel("com/bosssoft/platform/activiti/test/task/process_sourceSubsitution.bpmnx");
		...
		taskService.claimCascade(btaskId, getConsignInstIdForClaim(btaskId, "u03"), "u03");
		taskService.claimCascade(ctaskId, getConsignInstIdForClaim(ctaskId, "u03"), "u03");
		...
		taskService.unclaimCascade(ctaskId, getConsignInstIdForUmClaim(ctaskId, "u03"));
	...
	//	 taskService.completCascade(atask.getTaskId(), atask.getConsigInstId());
		 ...
		 taskService.claimCascade(atask.getTaskId(), getConsignInstIdForClaim(atask.getTaskId(), "u01"), "u01");
		 taskService.completCascade(atask.getTaskId(), getConsignInstIdForComplete(atask.getTaskId()));
	...
	public void  sourceCooperationRedo() throws Exception{
		...
		//	taskService.claimCascade(task.getTaskId(), instStr, "u03");
			...
			taskService.claimCascade(task.getTaskId(), getConsignInstIdForClaim(task.getTaskId(), "u01"), "u01");
			taskService.completCascade(task.getTaskId(), getConsignInstIdForComplete(task.getTaskId()));
		   ...
		   // taskService.redo(task.getConsigInstId());
		    taskService.redo(getConsignInstIdForRedo(task.getTaskId()));
